...
def calculate_triangle_distance(
    feed_dict, weights, binning, alternative_name, reference_name="herwig", ntrials=100
):
    w = np.abs(binning[1] - binning[0])
    x, _ = np.histogram(
        feed_dict[reference_name], weights=weights[reference_name], bins=binning
    )
    x2, _ = np.histogram(
        feed_dict[reference_name], weights=weights[reference_name] ** 2, bins=binning
    )

    x_norm = np.sum(x) * w
    y, _ = np.histogram(
        feed_dict[alternative_name], weights=weights[alternative_name], bins=binning
    )
    y2, _ = np.histogram(
        feed_dict[alternative_name],
        weights=weights[alternative_name] ** 2,
        bins=binning,
    )
    y_norm = np.sum(y) * w
    ...
    dist = sum(
        (
            0.5
            * w
            * (x[ib] / x_norm - y[ib] / y_norm) ** 2
            / (x[ib] / x_norm + y[ib] / y_norm)
            if x[ib] + y[ib] > 0
            else 0.0
        )
    ...
    for trial in range(ntrials):
        ...
        d_ = sum(
            (
                0.5
                * w
                * (x_[ib] / x_norm - y_[ib] / y_norm) ** 2
                / (x_[ib] / x_norm + y_[ib] / y_norm)
                if x_[ib] + y_[ib] > 0
                else 0.0
            )
            for ib in range(len(x))
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run the OmniFold training and plotting routine."
    )
    parser.add_argument(
        "--folder",
        type=str,
        default="/pscratch/sd/v/vmikuni/PET",
        help="Folder containing input files",
    )
    parser.add_argument(
        "--dataset", type=str, default="omnifold", help="Dataset to use"
    )
    parser.add_argument(
        "--mode", type=str, default="classifier", help="Loss type to train the model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_bins", type=int, default=50, help="Number of bins per histogram"
    )
    ...
    parser.add_argument(
        "--simple", action="store_true", help="Use simplified head model"
    )
    parser.add_argument(
        "--talking_head", action="store_true", help="Use talking head attention"
    )
    parser.add_argument(
        "--layer_scale",
        action="store_true",
...
def main():
    ...
    mfold = OmniFold(
        version="baseline", num_iter=flags.num_iter, checkpoint_folder=flags.folder
    )

    model_baseline = load_model(mc, flags, "baseline")
    unfolded_weights_baseline = reweight_samples(
        mc_inputs, model_baseline, mfold, batch_size=flags.batch
    )

    model_finetune = load_model(mc, flags, "fine_tune")
    unfolded_weights_finetune = reweight_samples(
        mc_inputs, model_finetune, mfold, batch_size=flags.batch
    )
    ...
    if hvd.rank() == 0:
        ...
        for feature in range(mc_features.shape[-1]):
            ...
            fig, ax, _ = plot_utils.HistRoutine(
                feed_dict,
                xlabel=name[feature],
            ...
            fig.savefig(
                "{}/omnifold_iter_{}_feat_{}.pdf".format(
                    flags.plot_folder, flags.num_iter, feature
                )
            )
