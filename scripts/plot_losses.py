...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Process and plot data based on the given dataset and configuration."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--plot_folder", type=str, default="../plots", help="Folder to save the outputs"
    )
    parser.add_argument(
        "--mode",
        type=str,
...
def load_and_plot_history(flags):
    ...
    fig, ax = plot_utils.PlotRoutine(
        plot_data,
        xlabel="Epochs" if loss_key == "val_loss" else "Epochs x 10",
        ylabel="Validation Loss",
        plot_min=True,
    )
    fig.savefig(
        f"{flags.plot_folder}/loss_{flags.dataset}_{flags.mode}.pdf",
        bbox_inches="tight",
    )
