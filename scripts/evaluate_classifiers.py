...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Evaluate performance metrics for trained models on various datasets."
    )
    parser.add_argument(
        "--dataset", type=str, default="top", help="Folder containing input files"
    )
    parser.add_argument(
        "--folder",
        type=str,
    ...
    parser.add_argument(
        "--load", action="store_true", help="Load pre-evaluated npy files"
    )
    parser.add_argument(
...
def print_metrics(y_pred, y, thresholds, multi_label=False):
    if multi_label:
        print(
            "AUC: {}".format(
                metrics.roc_auc_score(y, y_pred, average="macro", multi_class="ovo")
            )
        )
...
def load_or_evaluate_model(flags, test, folder_name):
    ...
    if flags.load:
        ...
        pred = hvd.allgather(
            tf.constant(model.predict(X, verbose=hvd.rank() == 0)[0])
        ).numpy()
        if hvd.rank() == 0:
