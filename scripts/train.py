...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Train the PET model on different datasets."
    )
    parser.add_argument(
        "--dataset", type=str, default="jetclass", help="Dataset to use"
    )
    parser.add_argument(
        "--folder",
        type=str,
    ...
    parser.add_argument(
        "--mode", type=str, default="all", help="Loss type to train the model"
    )
    ...
    parser.add_argument(
        "--stop_epoch", type=int, default=30, help="Epochs before reducing lr"
    )
    parser.add_argument("--lr", type=float, default=3e-5, help="learning rate")
    parser.add_argument("--wd", type=float, default=1e-5, help="weight decay")
    parser.add_argument(
        "--b1", type=float, default=0.95, help="beta1 for Lion optimizer"
    )
    parser.add_argument(
        "--b2", type=float, default=0.99, help="beta2 for Lion optimizer"
    )
    parser.add_argument(
        "--lr_factor", type=float, default=10.0, help="factor for slower learning rate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--drop_probability", type=float, default=0.0, help="Drop probability"
    )
    parser.add_argument(
        "--simple", action="store_true", default=False, help="Use simplified head model"
    )
    parser.add_argument(
        "--talking_head",
        action="store_true",
        default=False,
        help="Use talking head attention",
    )
    parser.add_argument(
        "--layer_scale",
        action="store_true",
...
def main():
    ...
    optimizer_body = configure_optimizers(
        flags, train_loader, lr_factor=flags.lr_factor if flags.fine_tune else 1.0
    )
    ...
    callbacks = [
        ...
        keras.callbacks.EarlyStopping(
            patience=flags.stop_epoch, restore_best_weights=True
        ),
        keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", patience=200, min_lr=1e-6
        ),
    ]
