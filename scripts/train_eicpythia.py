...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Train the PET model on EIC Pythia datasets."
    )
    ...
    parser.add_argument(
        "--mode", type=str, default="generator", help="Loss type to train the model"
    )
    parser.add_argument("--batch", type=int, default=512, help="Batch size")
    parser.add_argument("--epoch", type=int, default=520, help="Max epoch")
    parser.add_argument("--lr", type=float, default=3e-5, help="Learning rate")
    parser.add_argument(
        "--lr_factor", type=float, default=1.0, help="Factor to adjust learning rate"
    )
    parser.add_argument(
...
def get_data_loader(flags):
    train = utils.EicPythiaDataLoader(
        os.path.join(flags.folder, "EIC_Pythia", "train_eic.h5"),
        flags.batch,
        hvd.rank(),
        hvd.size(),
    )
    val = utils.EicPythiaDataLoader(
        os.path.join(flags.folder, "EIC_Pythia", "test_eic.h5"),
        flags.batch,
        hvd.rank(),
        hvd.size(),
    )
...
def configure_optimizers(flags, train_loader, lr_factor=1.0):
    ...
    optimizer = Lion(learning_rate=lr_schedule, clipnorm=1.0, beta_1=0.95, beta_2=0.99)
...
def main():
    ...
    optimizer_body = configure_optimizers(
        flags, train_loader, lr_factor=flags.lr_factor if flags.fine_tune else 1
    )
