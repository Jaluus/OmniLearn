...
labels = {"Pythia_26812400_10100000.h5"}
...
def process(p):
    ...
    new_p[:, :, 1] = p[:, 1:, 2]  # phi
    new_p[:, :, 2] = np.ma.log(
        np.ma.divide(p[:, 1:, 0], p[:, 0, 0, None]).filled(0)
    ).filled(
        0
    )  # pT
    new_p[:, :, 3] = np.sign(p[:, 1:, PID_INDEX])
    # hardcoded pids
    new_p[:, :, 4] = np.abs(p[:, 1:, PID_INDEX]) == 2212.0  # proton
    new_p[:, :, 5] = np.abs(p[:, 1:, PID_INDEX]) == 2112.0  # neutron
    new_p[:, :, 6] = np.abs(p[:, 1:, PID_INDEX]) == 321.0  # kaon+
    new_p[:, :, 7] = np.abs(p[:, 1:, PID_INDEX]) == 211.0  # pi +
    new_p[:, :, 8] = (
        (np.abs(p[:, 1:, PID_INDEX]) == 16.0)
        | (np.abs(p[:, 1:, PID_INDEX]) == 14.0)
        | (np.abs(p[:, 1:, PID_INDEX]) == 12.0)
    )  # neutrinos
    new_p[:, :, 9] = np.abs(p[:, 1:, PID_INDEX]) == 13.0  # muon
    new_p[:, :, 10] = np.abs(p[:, 1:, PID_INDEX]) == 11.0  # electron
    new_p[:, :, 11] = np.abs(p[:, 1:, PID_INDEX]) == 22.0  # photon
    new_p[:, :, 12] = np.abs(p[:, 1:, PID_INDEX]) == 130.0  # k0l
...
def preprocess(path, labels, nevent_max=-1, npart_max=-1):
    ...
    for label in labels:
        with h5.File(os.path.join(path, label), "r") as h5f:
            ...
            p, j = process(
                p
            )  # applies mask, saves real pid, shuffles feature indecies for training
            j = np.concatenate(
                [j, np.count_nonzero(p[:, :, 2], axis=-1, keepdims=True)], 1
            )  # pT moved to index 2
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--folder",
        type="string",
        default="/global/cfs/cdirs/m4662",
        help="Folder containing input files",
    )
    parser.add_option(
        "--nevent_max", type="int", default="10_000_000", help="max number of events"
    )
    parser.add_option(
        "--npart_max",
        type="int",
        default="50",
        help="max number of particses per event",
    )
    (flags, args) = parser.parse_args()

    train, val, test = preprocess(
        os.path.join(flags.folder, "diffusion"),
        labels,
        flags.nevent_max,
        flags.npart_max,
    )
    ...
    with h5.File(
        "{}/train_{}.h5".format(os.path.join(flags.folder, "diffusion"), "eic"), "w"
    ) as fh5:
    ...
    with h5.File(
        "{}/test_{}.h5".format(os.path.join(flags.folder, "diffusion"), "eic"), "w"
    ) as fh5:
    ...
    with h5.File(
        "{}/val_{}.h5".format(os.path.join(flags.folder, "diffusion"), "eic"), "w"
    ) as fh5:
