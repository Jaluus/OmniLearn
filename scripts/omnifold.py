...
class OmniFold:
    ...
    def RunStep1(self, i):
        ...
        norm_factor = hvd.allreduce(
            tf.constant(self.weights_pull, dtype=tf.float32)
        ).numpy()
    ...
    def RunModel(
        ...
        if hvd.rank() == 0:
            with open(
                model_name.replace("checkpoints", "histories").replace(
                    ".weights.h5", ".pkl"
                ),
                "wb",
            ) as f:
    ...
    def reweight(self, events, model, batch_size=None):
        if batch_size is None:
            batch_size = self.batch_size
        f = np.nan_to_num(
            expit(model.predict(events, batch_size=batch_size, verbose=0)[0]),
            posinf=1,
            neginf=0,
        )
        weights = f / (1.0 - f)
...
class Classifier(keras.Model):
    ...
    def train_step(self, inputs):
        ...
        self.body_optimizer.minimize(
            loss_pred, self.body.trainable_variables, tape=tape
        )
