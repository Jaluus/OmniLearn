...
def clustering(data, folder, sample="train.h5", nevents=1000, nparts=100):
    ...
    particles = np.stack(
        [
            data["fjet_clus_pt"][:nevents, :nparts] / 1000.0,
            data["fjet_clus_eta"][:nevents, :nparts],
            data["fjet_clus_phi"][:nevents, :nparts],
            data["fjet_clus_E"][:nevents, :nparts] / 1000.0,
        ],
        -1,
    )
    ...
    jets = np.stack(
        [
            data["fjet_pt"][:nevents] / 1000.0,
            data["fjet_eta"][:nevents],
            data["fjet_phi"][:nevents],
            data["fjet_m"][:nevents] / 1000.0,
        ],
        -1,
    )
    ...
    if "train" in sample:
        with h5py.File("{}/train_atlas.h5".format(folder), "w") as fh5:
            ...
            dset = fh5.create_dataset(
                "weights", data=weights[: int(0.8 * npid.shape[0])]
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=120, help="Number of particles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/PET/ATLASTOP",
        help="Folder containing input files",
    )
    parser.add_option(
        "--sample", type="string", default="test.h5", help="Input file name"
    )
