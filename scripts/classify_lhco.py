...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Classification script for LHCO data using PET classifier."
    )
    ...
    parser.add_argument(
        "--mode", default="classifier", help="Loss type to train the model"
    )
    parser.add_argument(
        "--SR", action="store_true", default=False, help="Generate SR data"
    )
    parser.add_argument("--batch", type=int, default=128, help="Batch size")
    parser.add_argument(
        "--epoch", type=int, default=10, help="Maximum number of epochs"
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=40,
        help="Number of steps in the cosine learning rate schedule",
    )
    parser.add_argument(
        "--nsig",
        type=int,
        default=1000,
        help="Number of signal events used in the training",
    )
    parser.add_argument("--lr", type=float, default=3e-5, help="Learning rate")
    parser.add_argument(
        "--lr_factor", type=float, default=10.0, help="Factor to adjust learning rate"
    )
    parser.add_argument(
        "--fine_tune", action="store_true", default=False, help="Fine tune a model"
    )
    parser.add_argument(
        "--ideal", action="store_true", default=False, help="Train idealized model"
    )
    parser.add_argument(
        "--nid", type=int, default=0, help="ID of the training for multiple runs"
    )
    parser.add_argument(
...
def main():
    ...
    optimizer_body = configure_optimizers(
        flags, train, lr_factor=flags.lr_factor if flags.fine_tune else 1
    )
