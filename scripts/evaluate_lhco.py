...
def parse_arguments():
    ...
    parser.add_argument(
        "--dataset", default="lhco", help="Folder containing input files"
    )
    parser.add_argument(
        "--folder",
    ...
    parser.add_argument(
        "--nevt", type=int, default=2000000, help="Number of events to generate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weighted",
        action="store_true",
        help="Load weights to correct model prediction",
    )
    parser.add_argument(
...
def plot(data, title, plot_folder, names, weights=None):
    ...
    for ivar in range(len(names)):
        ...
        fig, gs, _ = plot_utils.HistRoutine(
            feed_dict,
            xlabel=names[ivar],
            weights=weight_dict,
            plot_ratio=True,
...
def load_data_sample(flags):
    ...
    if flags.SR:
        ...
        y = test.LoadMjjFile(
            os.path.join(flags.folder, "LHCO"), "mjj_sample.h5", use_SR=True
        )[hvd.rank() : nevt : hvd.size()]
        del train
        jets = None
    else:
        y = test.y
    ...
    if flags.sample:
        ...
        file_names = {
            ...
            "true": os.path.join(
                flags.folder,
                "LHCO",
                "val_background_{}.h5".format("SR" if flags.SR else "SB"),
            ),
        }
        ...
        for file_name in file_names:
            ...
            mask = particles[file_name][:, 2] != 0.0
