...
def convert_to_np(file_list, base_path, name, max_part=30, nevts=30000000):
    ...
    for ifile, f in enumerate(file_list):
        ...
        mask_reco = np.stack(
            [tmp_file[feat].array()[mask_evt] for feat in mask_list], -1
        )
        ...
        pass_reco = (
            (data_dict["jet"][-1][:, 0] > 10)
            & (data_dict["jet"][-1][:, 1] > -1.5)
            & (data_dict["jet"][-1][:, 1] < 2.75)
        )
        data_dict["jet"][-1] = data_dict["jet"][-1][pass_reco]
        data_dict["data"][-1] = data_dict["data"][-1][pass_reco]
        ...
        mask_part = (
            (data_dict["data"][-1][:, :, 1] > -1.5)
            & (data_dict["data"][-1][:, :, 1] < 2.75)
            & (data_dict["data"][-1][:, :, -1] == 0)
        )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=30, help="Number of particles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/PET/H1",
    ...
    file_list = find_files_with_string(
        flags.folder + "/out_ep0607", "Django_Eplus0607_"
    )
    data_django = convert_to_np(
        file_list, flags.folder + "/out_ep0607", name="Django", max_part=flags.npoints
    )
    ...
    file_list = find_files_with_string(
        flags.folder + "/out_ep0607", "Rapgap_Eplus0607_"
    )
    data_rapgap = convert_to_np(
        file_list, flags.folder + "/out_ep0607", name="Rapgap", max_part=flags.npoints
    )
