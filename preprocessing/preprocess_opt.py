...
def clustering_sum(data_top, data_qcd, folder, nparts=100, is_train=False):
    ...
    npid = np.concatenate(
        [
            np.ones(data_top.shape[0], dtype=np.int32),
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=128, help="Number of partbicles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/PET/Opt/OptimalClassifierSamplesSameBinGen/top/raw",
        help="Folder containing input files",
    )
    parser.add_option(
        "--sample",
        type="string",
        default="train_nsamples1000000_trunc_5000_nonan.h5",
    ...
    data_qcd = h5py.File(os.path.join(samples_path.replace("top", "qcd"), sample))[
        "raw"
    ][:]
