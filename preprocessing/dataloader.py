...
def read_file(
    ...
    **Returns**
    ...
    table["log_part_e_rel"] = np.log(
        1e-6 + 1.0 - table["part_energy"] / table["jet_energy"]
    )
    ...
    x_particles = np.stack(
        [
            ak.to_numpy(_pad(table[n], maxlen=max_num_particles))
            for n in particle_features
        ],
        axis=-1,
    )
    x_jets = np.stack(
        [ak.to_numpy(table[n]).astype("float32") for n in jet_features], axis=-1
    )
