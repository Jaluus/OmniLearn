...
def preprocess(path, labels):
    ...
    for label in labels:
        with h5.File(os.path.join(path, label), "r") as h5f:
            ...
            p = process(p, j)
            pid = to_categorical(
                labels[label] * np.ones(shape=(j.shape[0], 1)), num_classes=5
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--label", type="string", default="150", help="Which dataset to use"
    )
    ...
    with h5.File(
        "{}/train_{}.h5".format(os.path.join(flags.folder, "JetNet"), flags.label), "w"
    ) as fh5:
    ...
    with h5.File(
        "{}/test_{}.h5".format(os.path.join(flags.folder, "JetNet"), flags.label), "w"
    ) as fh5:
    ...
    with h5.File(
        "{}/val_{}.h5".format(os.path.join(flags.folder, "JetNet"), flags.label), "w"
    ) as fh5:
