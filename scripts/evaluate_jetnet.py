...
def parse_arguments():
    ...
    parser.add_argument(
        "--dataset", default="jetnet150", help="Folder containing input files"
    )
    parser.add_argument(
        "--folder",
    ...
    parser.add_argument(
        "--skip_metric", action="store_true", help="Skip metric calculation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ideal", action="store_true", help="Use true jets for generation"
    )
    parser.add_argument(
...
def sample_data(test, model, flags, sample_name):
    ...
    if flags.ideal:
        ...
        file_name = os.path.join(
            flags.folder,
            "JetNet",
            "PET_jetnet150_8_local_layer_scale_token_baseline_generator.h5",
        )
...
def evaluate_metrics(
    particles, particles_gen, flavour, flavour_gen, sample_name, flags
):
    """Evaluate various metrics for the generated particles."""
    from jetnet.evaluation.gen_metrics import cov_mmd, fpnd, w1efp, w1m, w1p
    ...
    with open(sample_name.replace(".h5", ".txt"), "w") as f:
        for unique in np.unique(np.argmax(flavour, -1)):
            ...
            f.write(
                "{:.2f} $\pm$ {:.2f} & ".format(1e3 * np.mean(mean), 1e3 * np.mean(std))
            )
            mean_efp, std_efp = w1efp(particles[mask], particles_gen[mask_gen])
            print("W1EFP", 1e5 * np.mean(mean_efp), 1e5 * np.mean(std_efp))
            f.write(
                "{:.2f} $\pm$ {:.2f} & ".format(
                    1e5 * np.mean(mean_efp), 1e5 * np.mean(std_efp)
                )
            )
            if (
                flags.dataset == "jetnet150"
                or "w" in names[unique]
                or "z" in names[unique]
            ):
            ...
            cov, mmd = cov_mmd(
                particles[mask], particles_gen[mask_gen], num_eval_samples=1000
            )
...
def plot_results(jets, jets_gen, particles, particles_gen, flavour, flavour_gen, flags):
    ...
    flavour = np.tile(
        np.expand_dims(flavour, 1), (1, particles_gen.shape[1], 1)
    ).reshape((-1, flavour.shape[-1]))
    particles_gen = particles_gen.reshape((-1, 3))
    mask_gen = particles_gen[:, 2] > 0.0
...
def main():
    ...
    if flags.sample:
        ...
        jets_gen, particles_gen, flavour_gen = get_generated_data(
            sample_name, keep_top=flags.top
        )
        ...
        # Plot results
        plot_results(
            jets, jets_gen, particles, particles_gen, flavour, flavour_gen, flags
        )
