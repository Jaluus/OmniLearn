...
class DataLoader:
    ...
    def __init__(self, path, batch_size=512, rank=0, size=1, **kwargs):
        ...
        self.mean_jet = [6.18224920e02, 0.0, 1.2064709e02, 3.94133173e01]
    ...
    def make_tfdata(self):
        ...
        return (
            tf.data.Dataset.zip((tf_zip, tf_y))
            .cache()
    ...
    def load_data(self, path, batch_size=512, rank=0, size=1, nevts=None):
        ...
        # self.batch_size = batch_size
        self.nevts = (
            h5.File(self.path, "r")["data"].shape[0] if nevts is None else nevts
        )
...
class EicPythiaDataLoader(DataLoader):
    ...
    def __init__(self, path, batch_size=512, rank=0, size=1):
        ...
        def preprocess(self, x, mask):
            ...
            return (
                mask[:, :, None]
                * (new_x[:, :, :num_feat] - self.mean_part[:num_feat])
                / self.std_part[:num_feat]
            )
        ...
        self.steps_per_epoch = (
            None  # will pass none, otherwise needs to add repeat to tf data
        )
        self.files = [path]
    ...
    def revert_preprocess(self, x, mask):
        ...
        pids[
            np.arange(new_part.shape[0])[:, None],
            np.arange(new_part.shape[1]),
            max_indices,
        ] = 1
        new_part[:, :, 4:] = pids
...
class JetNetDataLoader(DataLoader):
    def __init__(self, path, batch_size=512, rank=0, size=1, big=False):
        ...
        if big:
            ...
            self.mean_jet = [1.0458962e03, 3.6804923e-03, 9.4020386e01, 4.9398304e01]
            ...
            self.mean_jet = [1.0458962e03, 3.6804923e-03, 9.4020386e01, 2.9036360e01]
...
class LHCODataLoader(DataLoader):
    def __init__(
        self, path, batch_size=512, rank=0, size=1, mjjmin=2300, mjjmax=5000, nevts=-1
    ):
        super().__init__(path, batch_size, rank, size)
    ...
    def get_mjj_mask(self, mjj, use_SR, mjjmin, mjjmax):
        if use_SR:
            ...
            mask_region = ((mjj < 3300) & (mjj > mjjmin)) | (
                (mjj > 3700) & (mjj < mjjmax)
            )
        return mask_region
    ...
    def pad(self, x, num_pad):
        return np.pad(
            x,
            pad_width=((0, 0), (0, 0), (0, 0), (0, num_pad)),
            mode="constant",
            constant_values=0,
        )
    ...
    def make_eval_data(self):
        ...
        return (
            tf_zip.cache().batch(self.batch_size).prefetch(tf.data.AUTOTUNE),
            self.label,
        )
    ...
    def make_tfdata(self, classification=False):
        ...
        if classification:
            tf_y = tf.data.Dataset.from_tensor_slices(
                np.concatenate([self.label, self.w], -1)
            )
        else:
            tf_y = tf.data.Dataset.from_tensor_slices(self.y)
        del self.X, self.y, self.mask
    ...
    def combine(self, datasets, use_weights=False):
        ...
        if use_weights:
            ...
            self.X, self.mask, self.jet, self.label, self.y = shuffle(
                self.X, self.mask, self.jet, self.label, self.y
            )
...
class TauDataLoader(DataLoader):
    def __init__(self, path, batch_size=512, rank=0, size=1, nevts=None):
        ...
        self.mean_jet = [6.16614813e01, 2.05619964e-03, 3.52885518e00, 4.28755680e00]
...
class AtlasDataLoader(DataLoader):
    def __init__(self, path, batch_size=512, rank=0, size=1, is_small=False):
        super().__init__(path, batch_size, rank, size)
        self.mean_jet = [1.73933684e03, 4.94380870e-04, 2.21667582e02, 5.52376512e01]
        self.std_jet = [9.75164004e02, 8.31232765e-01, 2.03672420e02, 2.51242747e01]
...
class OmniDataLoader(DataLoader):
    def __init__(self, path, batch_size=512, rank=0, size=1):
        ...
        self.mean_jet = [2.25826286e02, 1.25739745e-03, 1.83963520e01, 1.88828832e01]
        ...
        self.reco = self.get_inputs(
            self.X, h5.File(self.path, "r")["reco_jets"][rank::size]
        )
        self.gen = self.get_inputs(
            self.Y, h5.File(self.path, "r")["gen_jets"][rank::size]
        )
...
class JetClassDataLoader(DataLoader):
    ...
    def make_tfdata(self):
            ...
            output_signature=(
                {
                    "input_features": tf.TensorSpec(
                        shape=(self.num_part, self.num_feat), dtype=tf.float32
                    ),
        ...
        return (
            dataset.shuffle(self.batch_size * 50)
            .repeat()
            .batch(self.batch_size)
            .prefetch(tf.data.AUTOTUNE)
        )
