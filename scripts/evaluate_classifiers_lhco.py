...
def parse_options():
    ...
    parser.add_option(
        "--dataset", type="string", default="lhco", help="Folder containing input files"
    )
    parser.add_option(
        "--folder",
        type="string",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Load pre-evaluated npy files",
    )
    parser.add_option(
        "--mode",
        type="string",
        default="classifier",
        help="Loss type to train the model: available options are [all/classifier/generator]",
    )
    parser.add_option(
        "--fine_tune", action="store_true", default=False, help="Fine tune a model"
    )
    parser.add_option(
        "--nid", type=int, default=0, help="Training ID for multiple trainings"
    )
    parser.add_option(
        "--nsig", type=int, default=1000, help="Injected signal events for LHCO dataset"
    )
    # Model parameters
    parser.add_option(
        "--local", action="store_true", default=False, help="Use local embedding"
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Use average instead of class token",
    )
    parser.add_option(
        "--ideal",
        action="store_true",
        default=False,
        help="Load ideal training for LHCO",
    )
    parser.add_option(
        "--talking_head",
        action="store_true",
        default=False,
        help="Use talking head attention instead of standard attention",
    )
    parser.add_option(
        "--layer_scale",
        action="store_true",
...
def evaluate_existing_results(flags, folder_name, threshold):
    ...
    max_nid = 10
    sic, aucs = np.zeros((len(signal_values), max_nid)), np.zeros(
        (len(signal_values), max_nid)
    )
    for isig, nsig in tqdm(
        enumerate(signal_values), total=len(signal_values), desc="Processing Signals"
    ):
        for nid in range(max_nid):
            ...
            sic[isig, nid], aucs[isig, nid] = print_metrics(
                data.item()["pred"], data.item()["y"], threshold
            )
...
def save_results(flags, folder_name, model_name, y, pred):
    ...
    if not os.path.exists(npy_dir):
        os.makedirs(npy_dir)
    np.save(
        os.path.join(npy_dir, model_name),
        {
...
def display_statistics(sic, aucs):
    sic_median, sic_low, sic_high = (
        np.median(sic, -1),
        np.quantile(sic, 0.16, axis=-1),
        np.quantile(sic, 0.84, axis=-1),
    )
    auc_median, auc_low, auc_high = (
        np.median(aucs, -1),
        np.quantile(aucs, 0.16, axis=-1),
        np.quantile(aucs, 0.84, axis=-1),
    )
