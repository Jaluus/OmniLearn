...
def plot(jet1, jet2, flav1, flav2, nplots, title, plot_folder, is_big, names):
    ...
    for ivar in range(nplots):
        ...
        fig.savefig(
            "{}/Jetnet_{}_{}.pdf".format(plot_folder, title, ivar), bbox_inches="tight"
        )
...
def PlotRoutine(
    ...
    for ip, plot in enumerate(feed_dict.keys()):
        ...
        if plot_min:
            ...
            plt.axhline(
                y=min_val, linewidth=2, linestyle=line_style[plot], color=colors[plot]
            )
    ...
    if reference_name in feed_dict.keys():
        if weights is not None:
            reference_hist, _ = np.histogram(
                feed_dict[reference_name],
                weights=weights[reference_name],
                bins=binning,
                density=True,
            )
        else:
