...
def pad_and_combine(arrays, M):
    ...
    for iarr, array in tqdm(
        enumerate(arrays), total=len(arrays), desc="Processing arrays"
    ):
        P = array.shape[0]
...
def preprocess(data, folder, nparts=100, use_pid=True):
    ...
    y[y == 21] = 0
    jets = np.stack(
        [data.jets_f[:, 0], data.jets_f[:, 4], data.jets_f[:, 2], data.jets_f[:, 3]], -1
    )
    ...
    particles = np.asarray(
        [part[ef.mod.filter_particles(part, pt_cut=1)] for part in particles]
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=100, help="Number of particles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/QGCMS",
