...
def parse_arguments():
    ...
    parser.add_argument(
        "--mode",
        default="generator",
        help="Loss type to train the model: [all/classifier/generator]",
    )
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Sample from trained model"
    )
    parser.add_argument(
        "--plot_folder", default="../plots", help="Folder to save the outputs"
    )
...
def plot(jet1, jet2, var_names, title, plot_folder):
    for ivar in range(len(var_names)):
        feed_dict = {"eic_truth": jet1[:, ivar], "eic_gen": jet2[:, ivar]}

        fig, gs, binning = plot_utils.HistRoutine(
            feed_dict,
            xlabel=var_names[ivar],
            plot_ratio=True,
            logy=ivar == 0,
        ...
        fig.savefig(
            "{}/EIC_{}_{}.pdf".format(plot_folder, title, ivar), bbox_inches="tight"
        )
        plt.close()
...
def plot_results(jets, jets_gen, particles, particles_gen, flags):
    ...
    particles_gen = np.concatenate(
        [particles_gen, masses_gen[:, :, np.newaxis]], axis=-1
    )
    ...
    for pid in range(len(particle_names)):
        ...
        print(
            f"Fraction of {particle_names[pid]} in dataset: {1.0*np.sum(mask_pid)/mask_pid.shape[0]}"
        )
        mask_pid_gen = particles_gen[:, base_vars + add_vars + pid] == 1
        # Mask zero-padded particles
        particles_gen_pid = mask_pid_gen[:, None] * particles_gen
        particles_gen_pid = particles_gen_pid[particles_gen_pid[:, 4] != 0.0]
        particles_pid = mask_pid[:, None] * particles
        particles_pid = particles_pid[particles_pid[:, 4] != 0.0]

        var_names = [
            f"{particle_names[pid]}" + " $\log_{10}$(z)",
        ...
        plot(
            particles_pid[:, : len(var_names)],
            particles_gen_pid[:, : len(var_names)],
            title=f"Particle_{pid}",
            var_names=var_names,
            plot_folder=flags.plot_folder,
        )
    ...
    all_ele_gen = np.zeros(
        (particles_gen[:, 0][mask_electron_gen].shape[0] + len(jets_gen), 2)
    )
    all_ele_gen[:, 0] = np.concatenate(
        (particles_gen[:, 0][mask_electron_gen], jets_gen[:, 0])
    )
    all_ele_gen[:, 1] = np.concatenate(
        (particles_gen[:, 1][mask_electron_gen], jets_gen[:, -2])
    )
...
def main():
    ...
    if flags.sample:
        ...
        if hvd.rank() == 0:
            logging.info("Loading saved samples.")
