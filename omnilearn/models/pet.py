...
from ..nn.layers import LayerScale, RandomDrop, StochasticDepth, TalkingHeadAttention
...
class PET(keras.Model):
    ...
    def __init__(
        ...
        self.body = keras.Model(
            inputs=[input_features, input_points, input_mask, input_time],
        ...
        self.classifier_head = keras.Model(
            inputs=[outputs_body, input_jet],
            outputs=[outputs_classifier, outputs_regressor],
        )
        self.generator_head = keras.Model(
            inputs=[outputs_body, input_jet, input_mask, input_time, input_label],
            outputs=outputs_generator,
        )

        self.classifier = keras.Model(
            inputs=[input_features, input_points, input_mask, input_jet, input_time],
            outputs=[outputs_classifier, outputs_regressor],
        )
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if self.mode == "generator" or "all" in self.mode:
                ...
                eps = (
                    tf.random.normal((tf.shape(x["input_features"])), dtype=tf.float32)
                    * x["input_mask"][:, :, None]
                )
                ...
                perturbed_x = (
                    alpha[:, None] * x["input_features"] + eps * sigma[:, None]
                )
                perturbed_x = tf.where(
                    mask_diffusion, perturbed_x, tf.zeros_like(perturbed_x)
                )

                perturbed_body = self.body(
                    [perturbed_x, perturbed_x[:, :, :2], x["input_mask"], t]
                )
            ...
            if self.mode == "all":
                ...
                y_pred_smear, y_mse_smear = self.classifier_head(
                    [perturbed_body, x["input_jet"]]
                )
                loss_pred_smear = categorical_crossentropy(
                    y, y_pred_smear, from_logits=True
                )
        ...
        if self.mode == "all":
            trainable_vars = (
                self.classifier_head.trainable_variables
                + self.generator_head.trainable_variables
            )
            self.pred_tracker.update_state(y, y_pred)
            self.gen_tracker.update_state(loss_part)
        ...
        for weight, ema_weight in zip(
            self.generator_head.weights, self.ema_generator_head.weights
        ):
            ema_weight.assign(self.ema * ema_weight + (1 - self.ema) * weight)
    ...
    def PET_body(
        ...
        encoded = RandomDrop(
            self.feature_drop if "all" in self.mode else 0.0, num_skip=self.num_keep
        )(input_features)
        ...
        time = layers.Dense(2 * self.projection_dim, activation="gelu", use_bias=False)(
            time
        )
        ...
        if local:
            coord_shift = tf.multiply(
                999.0, tf.cast(tf.equal(input_mask, 0), dtype="float32")
            )
            points = input_points[:, :, :2]
            local_features = input_features
            for _ in range(num_local):
                local_features = get_neighbors(
                    coord_shift + points, local_features, self.projection_dim, K
                )
        ...
        for i in range(self.num_layers):
            ...
            if talking_head:
                updates, _ = TalkingHeadAttention(
                    self.projection_dim, self.num_heads, 0.0
                )(x1)
            else:
            ...
            if self.layer_scale:
                x3 = LayerScale(self.layer_scale_init, self.projection_dim)(
                    x3, input_mask
                )
    ...
    def PET_classifier(
        self, encoded, input_jet, num_class_layers, num_jet, simple=False
    ):
        ...
        if simple:
            ...
            representation = layers.Dense(self.projection_dim, activation="gelu")(
                representation + jet_encoded
            )
            outputs_pred = layers.Dense(
                self.num_classes, activation=self.class_activation
            )(representation)
            outputs_mse = layers.Dense(num_jet)(representation)
        else:
            conditional = layers.Dense(2 * self.projection_dim, activation="gelu")(
                input_jet
            )
            ...
            class_tokens = tf.Variable(
                tf.zeros(shape=(1, self.projection_dim)), trainable=True
            )
            class_tokens = tf.tile(
                class_tokens[None, :, :], [tf.shape(encoded)[0], 1, 1]
            )
    ...
    def PET_generator(self, encoded, input_jet, label, time, mask, num_layers, simple):

        time = FourierProjection(time, self.projection_dim)
        cond_jet = layers.Dense(self.projection_dim, activation="gelu")(input_jet)
        cond_token = layers.Dense(2 * self.projection_dim, activation="gelu")(
            tf.concat([time, cond_jet], -1)
        )
        ...
        cond_token = (
            tf.tile(cond_token[:, None, :], [1, tf.shape(encoded)[1], 1]) * mask
        )
        ...
        if simple:
            ...
            encoded = (
                layers.GroupNormalization(groups=1)(encoded) * (1.0 + scale) + shift
            )
...
def FourierProjection(x, projection_dim, num_embed=64):
    ...
    embedding = layers.Dense(2 * projection_dim, activation="swish", use_bias=False)(
        embedding
    )
    embedding = layers.Dense(projection_dim, activation="swish", use_bias=False)(
        embedding
    )
