...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Train the Classifier model on the ATLAS dataset."
    )
    ...
    parser.add_argument(
        "--stop_epoch", type=int, default=5, help="Stop epoch for early stopping"
    )
    parser.add_argument("--lr", type=float, default=3e-5, help="Learning rate")
    parser.add_argument(
        "--lr_factor", type=float, default=5.0, help="factor for slower learning rate"
    )
    parser.add_argument(
...
def main():
    ...
    callbacks = [
        ...
        keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", patience=1000, min_lr=1e-6
        ),
    ]
