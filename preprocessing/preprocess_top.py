...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=150, help="Number of particles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/PET/TOP/",
        help="Folder containing input files",
    )
    parser.add_option(
        "--sample", type="string", default="train.h5", help="Input file name"
    )
    ...
    clustering_sum(
        data, samples_path, flags.sample.replace(".h5", ""), data.shape[0], NPARTS
    )
