...
def preprocess(p, jets, folder, nparts=100):
    ...
    points[:, :, 8] = (
        (np.abs(pid) == 211) | (np.abs(pid) == 321) | (np.abs(pid) == 2212)
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--npoints", type=int, default=101, help="Number of particles per event"
    )
    parser.add_option(
        "--folder",
        type="string",
        default="/pscratch/sd/v/vmikuni/PET/OmniFold",
    ...
    if flags.sample == "pythia":
        dataset_pythia = ef.zjets_delphes.load(
            "Pythia21",
            num_data=-1,
            pad=True,
        ...
        reco_parts, reco_jets = preprocess(
            dataset_pythia["sim_particles"],
            dataset_pythia["sim_jets"],
            samples_path,
            flags.npoints,
        )
        gen_parts, gen_jets = preprocess(
            dataset_pythia["gen_particles"],
        ...
        dataset_herwig = ef.zjets_delphes.load(
            "Herwig",
            num_data=-1,
            pad=True,
            cache_dir=samples_path,
        ...
        reco_parts, reco_jets = preprocess(
            dataset_herwig["sim_particles"],
            dataset_herwig["sim_jets"],
            samples_path,
            flags.npoints,
        )
        gen_parts, gen_jets = preprocess(
            dataset_herwig["gen_particles"],
