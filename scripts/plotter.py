...
def parse_options():
    ...
    parser.add_argument(
        "--n_bins", type=int, default=50, help="Number of bins for the histograms"
    )
...
def main():
    ...
    for feat in range(len(test.jet_names)):
        ...
        fig.savefig(
            f"{flags.plot_folder}/jets_{flags.dataset}_{feat}.pdf", bbox_inches="tight"
        )
    ...
    for feat in range(len(test.part_names)):
        ...
        fig, gs, _ = plot_utils.HistRoutine(
            {"{}".format(flags.dataset): flat},
            test.part_names[feat],
            "Normalized Events",
            plot_ratio=False,
            reference_name="{}".format(flags.dataset),
        )
        fig.savefig(
            f"{flags.plot_folder}/parts_{flags.dataset}_{feat}.pdf", bbox_inches="tight"
        )
